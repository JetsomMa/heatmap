...
AFRAME.registerComponent('aframe-heatmap3d', {
  ...
  update: function (oldData) {
    ...
    if(Object.keys(diff).length===0||!data.show) return;
    ...
    if (data.updateGeometry) {
      ...
      if(data.points.length > 0){ //点阵数据初始化
        ...
        var maxValue = 0;
        var itemNumber = 0;
        if(data.onlyPointsValue){
          data.points.forEach((item)=>{if(item > maxValue) maxValue=item})
          this.heights = data.points
        } else {
          this.points = data.points;
          this.heights = data.points.map(
            (item)=>{
        ...
        this.heights = this.heights.map(item=>item/maxValue)
        ...
        this.geometry = new Heatmap3dPlaneBufferGeometry(data.widthStep, data.heightStep, data.widthPointNumber, data.heightPointNumber, this.heights, data.ignoreZeroValues, data.ignoreTransparentValues, this.points);
        ...
        //对点位进行高斯模糊处理
        let imgData = {data: this.heights, width: this.canvas.width-1, height: this.canvas.height-1}
        imgData = gaussBlur(imgData, 3)
        this.heights = imgData.data
    ...
    data.updateMaterial = (this.material === undefined || "emissive" in diff || "points" in diff || "flipPalette" in diff || "roughness" in diff || "metalness" in diff || "shininess" in diff || "emissiveIntensity" in diff || "opacityMin" in diff || "opacityMax" in diff || "palette" in diff || "scaleOpacityMethod" in diff || "scaleOpacity" in diff || "wireframe" in diff || "renderMode" in diff || "particleSize" in diff);
...
function Heatmap3dPlaneBufferGeometry(width, height, widthSegments, heightSegments, vals, skipZeros, skipNegative, points) {
